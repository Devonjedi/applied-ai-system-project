...
def classify_song(song: Song, profile: Dict[str, object]) -> str:
    ...
    # Fix: chill keywords should match against genre, not title (consistent with is_hype_keyword)
...
def compute_playlist_stats(playlists: PlaylistMap) -> Dict[str, object]:
    ...
    # Fix: ratio should divide by total songs, not just hype count (was always 1.0)
    total = len(all_songs)
    hype_ratio = len(hype) / total if total > 0 else 0.0

    avg_energy = 0.0
    if all_songs:
        # Fix: average energy should sum all songs, not just hype songs
...
def search_songs(
    ...
    for song in songs:
        ...
        # Fix: query should be searched inside the field value, not the other way around
        if q in value:
...
def random_choice_or_none(songs: List[Song]) -> Optional[Song]:
    ...
    # Fix: random.choice crashes on an empty list; return None instead
    if not songs:
        return None
